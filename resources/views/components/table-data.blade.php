            ...
            <thead class="{{ $conf['c_thead'] }}">
                <tr>
                    @foreach ($conf['ths'] as $k => $th)
                        <th class="text-uppercase font-weight-bolder {{ $conf['c_ths'][$k] }}"
                            width="{{ $conf['w_ts'][$k] }}%">{{ $th }}</th>
                    @endforeach
            ...
            <tbody id="body-table">
                @for ($o = 0; $o < count($conf['data']); $o++)
                    @if ($conf['show'] == true)
                        <tr style="cursor:pointer;"
                            onclick="window.location='{{ $conf['url'] }}/{{ $conf['data'][$o][$conf['id']] }}';">
                        @elseif ($conf['edit'] == true)
                            @if (Gate::check($conf['group'] . '-edit') || Gate::check('adm-edit'))
                        <tr style="cursor:pointer;"
                            onclick="window.location='{{ $conf['url'] }}/{{ $conf['data'][$o][$conf['id']] }}/edit';">
                    @endif
                @else
                    <tr>
                @endif
                ...
                @for ($oa = 0; $oa < count($conf['tds']); $oa++)
                    <td class="{{ $conf['c_ths'][$oa + 1] }}">
                        @if (is_string($conf['data'][$o][$conf['tds'][$oa]]) == false &&
                            is_null($conf['data'][$o][$conf['tds'][$oa]]) == false)
                            @if ($conf['td_number'][$oa] == false)
                ...
                @if ($conf['edit_modal'] == true)
                    @if (Gate::check($conf['group'] . '-edit') || Gate::check('adm-edit'))
                        <td width="3%" class="{{ $conf['c_ths'][0] }}"><a class="btn btn-warning mb-0"
                                id="editCompany" onclick="editModal('{{ $conf['data'][$o][$conf['id']] }}');"><i
